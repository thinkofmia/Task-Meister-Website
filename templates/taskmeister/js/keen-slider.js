...
function createKeenSlider(urls)
{
    // return from function if urls is an empty array or only contains strings with whitespace
    if(!/\S+/.test(urls))
        return;
    nav_wrapper = document.createElement("div");
    nav_wrapper.className = "navigation-wrapper";

    // create slider div
    my_keen_slider = document.createElement("div");
    my_keen_slider.id = "my-keen-slider";
    my_keen_slider.className = "keen-slider";
    ...
    path_left.setAttributeNS(null, "d", "M16.67 0l2.83 2.829-9.339 9.175 9.339 9.167-2.83 2.829-12.17-11.996z");
    arrow_left.appendChild(path_left);
    ...
    path_right.setAttributeNS(null, "d", "M5 3l3.057-3 11.943 12-11.943 12-3.057-3 9-9z");
    arrow_right.appendChild(path_right);

    dots = document.createElement("div");
    dots.id = "dots";
    dots.className = "dots";

    // add child elements to wrapper
    [my_keen_slider, arrow_left, arrow_right, dots].forEach(e => nav_wrapper.appendChild(e));

    // append nav_wrapper to nearest article tag
    document.querySelector("article").appendChild(nav_wrapper);

    // populate slider with media
    urls.forEach(url => createSlide(url));

    return newSlider("#my-keen-slider");
}

function newSlider(slider)
{
    return new KeenSlider(slider, {
        loop: true,
